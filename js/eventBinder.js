...
import {
    ...
    currentTaskLi // Importa a variável exportada
} from './uiActions.js'; 
...
export function setupCommonEventListeners() {
    ...
    $(document).on('click', '.remove-btn', function () {
        ...
        $('#confirm-modal-ok-btn').off('click').on('click', async () => { await deleteTask(taskId); hideModal('#confirmModal'); });
    ...
    $(document).on('click', '.subtask-options-btn', function(e) {
        ...
        toggleSubtaskMenu($(this), taskId, subtaskId, parentId); // Chama a função que está em uiActions.js
    });
    
    // --- Listener para o botão de Opções da TAREFA PRINCIPAL ---
    ...
    $(document).on('click', '.google-calendar-btn', function () {
        ...
        if (task) exportTaskToGoogleLink(task); // Chama a função que está em uiActions.js
    });
    ...
    // --- Lixeira ---
     ...
     $(document).on('click', '.btn-delete-permanently', function() {
        ...
        $('#confirm-modal-ok-btn').off('click').on('click', async () => { await permanentlyDeleteTask(taskId); hideModal('#confirmModal'); });
        $('#confirm-modal-cancel-btn').off('click').on('click', () => hideModal('#confirmModal'));
     });
